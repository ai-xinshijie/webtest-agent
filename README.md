...
- BFS 页面探索、逐组件动作覆盖、n-wise 组合覆盖、路径覆盖、状态迁移记录、语义 Oracle 与深度网络故障注入。业务组件与导航、页面外壳、第三方组件分开统计；不可见或禁用控件会先尝试恢复可达性，仍不可达才记录为受阻覆盖。
...
组合空间无限大时不存在数学意义上的“绝对穷尽”。本项目对小空间执行全组合；大空间执行有界 t-way 覆盖、路径覆盖和预算化发散。报告会分别列出动作、组合、路径的本会话实际执行、复用、受阻和待覆盖数；实际覆盖率不把历史结果算作本次执行，已解析率才包含页面指纹未变更时的复用和已确认阻断，不能用单一百分比掩盖边界。
...
- `report-<session>.md`：中文测试报告、问题清单、组件范围、覆盖快照、状态图、代码自愈与时间线。
