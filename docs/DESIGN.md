...
| FR-01-20 | `wta case list/show/run` | 查看编译用例、中文步骤与断言，并精确重跑单个用例 |
...
| FR-02-07 | Cases | 已编译用例列表：步骤、断言、执行状态和单条重跑 |
...
- 覆盖矩阵（页面 × 组件 × 测试类型）
- 测试摘要（耗时、通过/失败/跳过）
- 编译用例、中文步骤、断言和最近执行状态；Markdown / JSON 内容可在 GUI 直接预览
- 导出按钮（Markdown / JSON）
...
- 组件按业务、导航、页面外壳、第三方和未知范围分类；只有业务组件进入主覆盖率，其余范围作为可审计探索证据单独报告。动作覆盖对每个可识别组件生成类型化动作；不可见或禁用项先通过滚动、状态检查和 `trial click` 恢复可达性，无法恢复才记录为暂时或永久受阻，绝不使用 `force` 绕过真实用户限制。
- 小组合空间执行完整笛卡尔组合；大空间执行有限强度的 t-way 覆盖数组。路径覆盖采用无循环、最大深度和最大路径数的明确预算。
- 探索结束即由持久化页面、组件和类型化动作编译为可重放用例。每个用例包含导航、动作、断言三步、selector、导航宏和执行状态；探索会话不执行功能动作，但可以直接审阅其生成的测试计划。`wta case` 与 GUI “用例”页都能显示步骤并单条重跑；单条重跑固定为 `retest + test`，只执行选中动作。
- `continue`、`fresh`、`retest`、`expand`、`regression` 通过记忆库确定跳过、重测或扩展范围。历史成功动作只在当前页面指纹不变时复用，并单独计入复用而非本会话实际执行；路径永远重新执行。完成后压缩为可继承摘要。
- 每次动作保存前后状态节点与迁移证据，确定性语义 Oracle 校验页面稳定性、无效提交不可绕过校验、有效提交有可观察反馈；失败 Oracle 以可定位质量规则写入问题列表。
...
Web 界面的状态与组合空间通常是无界的，不能诚实地宣称对任意系统“绝对穷尽”。本实现对可发现的同源页面、已识别组件与设定预算内路径给出可审计的覆盖快照；动作、组合、路径分别列出本会话已执行、复用、受阻和待覆盖，组件范围也单列。实际覆盖率不计历史复用，已解析率包含页面指纹一致的复用和已确认受阻。状态图则记录已发现状态节点与通过/失败迁移。需要扩大探索时使用 `--mode expand` 或提高目标策略预算。
