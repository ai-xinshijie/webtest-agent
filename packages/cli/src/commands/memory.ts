...
function withManager<T>(action: (manager: MemoryManager) => T): T {
...
memoryCommand
  ...
  .action((options: { o: string; app?: string }) => {
    const data = withManager(manager => manager.export(options.app));
...
memoryCommand
  ...
  .action((file: string) => {
    ...
    const count = withManager(manager => manager.import(data));
    console.log(`已导入 ${count} 条记忆记录`);
  });
...
memoryCommand
  ...
  .action((files: string[], options: { o: string }) => {
    ...
    const merged = withManager(manager => manager.merge(inputs));
...
memoryCommand
  ...
  .action((options: { app?: string; all?: boolean }) => {
    ...
    withManager(manager => manager.clear(options.app));
